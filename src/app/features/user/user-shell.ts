...
import { ROLE_BRANCH_ADMIN } from '../../core/auth/models';
...
export class UserShell {
  ...
  /** Hides role-gated items (e.g. "My Team") from anyone who doesn't have that role — see NavItem.roles' own doc comment. */
  protected readonly navItems = computed(() => {
    const roles = this.authService.currentUser()?.roles ?? [];
    return USER_NAV_ITEMS.filter((item) => !item.roles || item.roles.some((r) => roles.includes(r)));
  });
