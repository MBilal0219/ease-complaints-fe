...
@Component({
  ...
  template: `
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div>
        <h1 class="text-lg font-semibold text-slate-900">Sales People</h1>
        <p class="mt-1 text-sm text-slate-500">Team members who contact customers, record calls, and manage leads.</p>
      </div>
      <button
        type="button"
    ...
    <div class="mt-6 overflow-hidden rounded-lg border border-slate-200 bg-white">
      <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 p-4">
        <h2 class="text-sm font-semibold text-slate-900">All sales people ({{ result().totalCount }})</h2>
        <div class="relative">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
              @for (salesPerson of result().items; track salesPerson.id) {
                <tr class="cursor-pointer hover:bg-slate-50" [routerLink]="['/app/admin/sales-people', salesPerson.id]">
                  <td class="px-4 py-2.5 font-medium text-slate-900">{{ salesPerson.displayName }}</td>
                  <td class="px-4 py-2.5 text-slate-600">{{ salesPerson.email }}</td>
                  <td class="px-4 py-2.5">
                    <span
                      class="rounded-full px-2 py-0.5 text-xs font-medium"
                      [class]="salesPerson.isActive ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'"
                    >
                      {{ salesPerson.isActive ? 'Active' : 'Inactive' }}
                    </span>
                  </td>
                  <td class="px-4 py-2.5 text-slate-600">{{ salesPerson.createdAtUtc | date: 'mediumDate' }}</td>
                  <td class="px-4 py-2.5 text-slate-600">
                    {{ salesPerson.lastLoginAtUtc ? (salesPerson.lastLoginAtUtc | date: 'medium') : 'Never' }}
                  </td>
                </tr>
              } @empty {
                <tr>
                  <td colspan="5" class="px-4 py-8 text-center text-slate-500">No sales people yet.</td>
                </tr>
              }
            </tbody>
          </table>
...
export class SalesPeoplePage implements OnInit {
  ...
  ngOnInit(): void {
    merge(timer(0, POLL_MS), this.manualRefresh)
      .pipe(
        switchMap(() => this.adminService.getSalesPeople(this.searchTerm(), this.page(), PAGE_SIZE).pipe(catchError(() => of(null)))),
